import numpy as np
import cv2

###img = cv2.imread("messi5.jpg",1)###

#we can also creat a blank image by numpy method
img = np.zeros([512,512,3],np.uint8)
#Zeros is method pro crating blank image
#[512,512,3] height width 3*default it can be from 1 to 4 but prefer 3
#np.uint8 is data type
img = cv2.line(img, (0,0), (255,255), (102,153,255),10)

img = cv2.arrowedLine(img, (0,255), (255,255), (0,255,0),10)


img = cv2.rectangle(img, (384,0), (510,128), (0,0,255),20)
img = cv2.circle(img, (447,63), 63, (0,255,0),-1)


font = cv2.FONT_HERSHEY_COMPLEX
img = cv2.putText(img, "Open CV", (10,320), font, 2, (0,255,255),10,cv2.LINE_AA)


cv2.imshow("image", img)

cv2.waitKey(0)
cv2.destroyAllWindows()